...
# Let's start by loading package pryr for easier navigation through object types
# library(pryr)
...
# - more rigorous class definition
# - a set of functions which allows for class creation: 
#     - setClass() for class definition
#     - new() for creating new objects

# Class attributes in S4 are called slots - we call them with @ symbol

# When defining a class we need to provide its name and names of individual slots.
# Slots need to have their type defined (structure requirements).



### New S4 class ###############################################################

# Let's define the class called "client4" - equivalent to the class "client" from 
...
# then let's create a new object of this class using the new() function

# we must provide the class name and values of individual slots

k4 <- new(Class = "client4", # Capital letter here!!
          fname = "John",
          lname = "Smith",
          age = 35,
          gender = "M",
...
# isS4() function allows to check if the object has the S4 class
...
### Constructor named as the class #############################################

# Storing the result of the setClass() function to a new object will create a 
# function that can be used to create new objects of this class 
# (it is called a constructor / generator function).
...
# also include data (slots) validation rules that are more detailed than 
# just the type checking!!
...
# modification of particular slots is possible with direct references

k4@age <- 140

# unfortunately in this case the correctness of inserted data is not verified

# we can check if the object is valid with the validObject() function
...
# alternatively to view and modify field values we can use the slot() function
# here also the correctness of the value is not automatically verified:
...
# In S4 methods belong to generic functions and not to class definitions (like in S3)

# Creating methods and generic functions will be very similar as in S3.
# We will just use dedicated functions setGeneric() and setMethod().
...
# then for S4 system objects the appropriate show() method will be called, 
# while for a object of S3 system - print() method
...
### Creating method in S4 for existing generic #################################
...
### Methods working on multiple objects ########################################

# An advantage of the S4 system, apart from the formal definition of classes, 
# it the possibility to define multi-argument methods, which is not possible with 
# the S3 system.
...
clientPremium <- setClass("clientPremium", 
         
         # contains allows for inheriting slots from other classes
         contains = "client4",
         
         # new slots (unique to our new class) can be defined in typical slots argument
         slots = list(
           premium = "logical"
         ),
         
         # prototype allows for defining the default slot values
         prototype = list( 
           premium = TRUE
         ))


kP <- clientPremium(fname = "Anne",
              lname = "Reeves",
              age = 24, 
              gender = "F",
              married = FALSE,
...
kP <- clientPremium(fname = "Anne",
                    ...
                    gender = "F",
                    married = FALSE) 
# we can skip the premium parameter, as we have specified its default value in prototype


# we can check the class of our object with standard class function
class(kP) # here we cannot see the inheritance as it does not recognise S4 inheritance

is(kP) # shows the proper S4 class definition
# Again we can see more specific class at the begining (clientPremium)
# and the parent class later on


kP # if we call clientPremium object automatically show method for client4 is used

# We can also use the compare function which was defined for client4 objects
compare(k4, kP)



# We can define new, specific show method for clients premium

setMethod(f = "show", # method
          signature = "clientPremium", 
          definition = function(object) {
            cat(object@fname, object@lname, "\n")
            cat("age:", object@age, "years\n")
            cat("gender:", object@gender, "\n")
            # print if married and go to the new line 
            cat("married:", ifelse(object@married, "Yes", "No"), "\n")
            cat("client premium:", ifelse(object@premium, "Yes", "No"))
          }
)

kP

# It works as in S3 - if a function is defined for the child class
# it will be used as a primary call. However, if we don't have the method
# for the child class, parent's method will be used. 

# S4 supports multiple inheritance and more nuanced ways of inheriting 
# methods and slots form parent's classes. You can read about it in more
# detail here: https://adv-r.hadley.nz/s4.html#inheritance-1 
# and here: https://adv-r.hadley.nz/s4.html#s4-dispatch 
...
# Exercise 1
# (partial equivalent of the Exercise 3 from previous lab)
