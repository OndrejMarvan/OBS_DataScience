...
We call the line describing the relationship the **regression line**. We have a line only for a model with an intercept and one explanatory variable. With two explanatory variables, the result is a **regression plane**. With many variables, we speak of a **regression hyperplane**.