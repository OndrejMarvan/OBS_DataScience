...
par(mfrow=c(2,1)) #to have two graphs in one window one above the other (par(mfrow=c(1,2)) - next to each other)
...
plot_colorByDensity(alfa_b,beta_b,xlab="a",ylab="b",
                    main = paste0("XY plot of bootstrapped estimators [mean_a =", round( mean(alfa_b),1), ", mean_b =", round( mean(beta_b),2), "]"))
