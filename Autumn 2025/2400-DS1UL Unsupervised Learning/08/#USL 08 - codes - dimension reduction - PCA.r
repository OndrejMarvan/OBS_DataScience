...
# data - same as in the case of MDS
...
preproc1 <- preProcess(price_what, method=c("center", "scale"))
price_what.s <- predict(preproc1, price_what)
 
summary(price_what.s)

# eigenvalues on the basis of covariance
price_what.cov<-cov(price_what.s)
price_what.eigen<-eigen(price_what.cov)
price_what.eigen$values
head(price_what.eigen$vectors)

# Other interesting resources on-line
# https://rpubs.com/williamsurles/310847
# https://cran.r-project.org/web/packages/onlinePCA/onlinePCA.pdf
...
xxx<-price_what.s # for easier references 
...
# https://kb.osu.edu/dspace/bitstream/handle/1811/69494/ASK_1998_83_102.pdf 
...
library(factoextra)
km3<-eclust(price_what, k=10) # 10 clusters for observations
...
#km3<-eclust(price_what, k=10) # 10 clusters for observations, from factoextra::
