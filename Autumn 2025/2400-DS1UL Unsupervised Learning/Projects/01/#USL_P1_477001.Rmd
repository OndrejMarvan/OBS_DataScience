...
# 1. Prepare data for clustering: Scale ONLY the energy columns
# We use the renamed columns from your previous step
energy_for_clustering <- energy_research %>%
  select(Coal, Oil, Gas, Nuclear, Hydropower, Wind, Solar, `Other renewables`) %>%
  scale()

# 2. Elbow Method (Within-cluster sum of squares)
fviz_nbclust(energy_for_clustering, kmeans, method = "wss") +
  geom_vline(xintercept = 5, linetype = 2, color = "red") +
  labs(title = "Elbow Method", subtitle = "Checking the 'bend' at k=5")

# 3. Silhouette Method (Cluster separation quality)
fviz_nbclust(energy_for_clustering, kmeans, method = "silhouette") +
  labs(title = "Silhouette Method", subtitle = "Higher is better")
```
