...
Before applying unsupervised learning algorithms, I must inspect the raw data. Possibly to delete outliers. 
...
head(energy_data) # Entity 
dim(energy_data) # 6585 rows, 11 columns
str(energy_data) 

colnames(energy_data)
```
```{r drop code, include=TRUE}
df_no_code <- energy_data[, names(energy_data) != "Code"]
colnames(energy_data)
```
