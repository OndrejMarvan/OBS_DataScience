...
**Dataset:** MovieLens (Harper & Konstan, 2015) - 9,742 movies with genre labels.

https://grouplens.org/datasets/movielens/latest/

**Note**: I am using subset instead of full dataset. Comparisson: 

1. Small version: 100,000 ratings and 3,600 tag applications applied to 9,000 movies by 600 users. Last updated 9/2018

2. Full: approximately 33,000,000 ratings and 2,000,000 tag applications applied to 86,000 movies by 330,975 users. Includes tag genome data with 14 million relevance scores across 1,100 tags. Last updated 9/2018.
...
ggplot(genre_counts, aes(x = reorder(genres, n), y = n)) +
  ...
  coord_flip() +
  labs(title = "Genre Frequency", x = "", y = "Number of Movies")
```
...
movies$n_genres <- str_count(movies$genres, "\\|") + 1   # count | and add 1

summary(movies$n_genres)

# Histogram
ggplot(movies, aes(x = n_genres)) +
  geom_histogram(binwidth = 1, fill = "steelblue", color = "white") +
  labs(title = "Number of Genres per Movie", x = "Genres", y = "Movies")
```
...
genre_list <- strsplit(movies_clean$genres, "\\|")

# Convert to transactions
trans <- as(genre_list, "transactions")
...
itemFrequencyPlot(trans, topN = 15, type = "absolute", 
                  main = "Genre Frequency in Transactions")
```
...
1. Some genres often appear together (e.g., Action-Adventure, Comedy-Romance)
...
- High lift = genres are realy connected, not just common
